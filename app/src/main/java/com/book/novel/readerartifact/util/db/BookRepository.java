...
public class BookRepository {
    ...
    private CollectBookBeanDao mCollBookDao;
    private BookRepository(){
        mSession = DaoDbHelper.getInstance()
                .getSession();
        mCollBookDao = mSession.getCollectBookBeanDao();
    }
    ...
    //存储已收藏书籍
    public void saveCollBookWithAsync( CollectBookBean bean){
        //启动异步存储
     ...
     * @param beans
     */
    public void saveCollBooksWithAsync(List< CollectBookBean> beans){
        mSession.startAsyncSession()
                .runInTx(
                        () -> {
                            for ( CollectBookBean bean : beans){
                                if (bean.getBookChapters() != null){
    ...
    public void saveCollBook( CollectBookBean bean){
        mCollBookDao.insertOrReplace(bean);
    }

    public void saveCollBooks(List< CollectBookBean> beans){
        mCollBookDao.insertOrReplaceInTx(beans);
    }
    ...
    /*****************************get************************************************/
    public  CollectBookBean getCollBook(String bookId){
        CollectBookBean bean = mCollBookDao.queryBuilder()
                .where( CollectBookBeanDao.Properties._id.eq(bookId))
                .unique();
        return bean;
    }
    ...
    public  List< CollectBookBean> getCollBooks(){
        return mCollBookDao
                .queryBuilder()
                .orderDesc( CollectBookBeanDao.Properties.LastRead)
                .list();
    }
    ...
    public ChapterInfoBean getChapterInfoBean(String folderName, String fileName){
        File file = new File(CommonString.BOOK_CACHE_PATH + folderName
    ...
    /************************************************************/
    public Single<Void> deleteCollBookInRx( CollectBookBean bean) {
...
//                e.onSuccess(new Void());
    ...
    public void deleteCollBook( CollectBookBean collBook){
        mCollBookDao.delete(collBook);
    }

    //删除书籍
    public void deleteBook(String bookId){
        FileUtils.deleteFile(CommonString.BOOK_CACHE_PATH+bookId);
    }
